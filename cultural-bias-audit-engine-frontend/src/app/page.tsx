...
export default function Dashboard() {
  ...
  const [activeTabs, setActiveTabs] = useState<
    Record<string, "corporate" | "formal" | "informal">
  >({});
  ...
  const handleAudit = async (e?: React.FormEvent) => {
    ...
    try {
      ...
      if (err instanceof Error) {
        ...
        setError(
          "Failed to connect to FastAPI backend. Ensure Uvicorn is running on port 8000.",
        );
      }
    } finally {
      setLoading(false);
      ...
      <div className="max-w-6xl mx-auto space-y-8">
        ...
        <header className="space-y-3 border-b border-slate-800 pb-6">
          ...
          <p className="text-slate-400 max-w-3xl text-sm sm:text-base leading-relaxed">
            Benchmarking generative LLM outputs for microaggressions, dialectal
            erasure (AAVE), implicit pragmatic subtext, and multi-register
            remediation.
          </p>
        </header>
            ...
            <div className="bg-slate-800 border border-slate-700 rounded-2xl p-6 shadow-xl flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
              <div className="space-y-1">
                ...
                <p className="text-sm text-slate-400">
                  Aggregated average across microaggression and dialectal
                  erasure indices.
                </p>
              </div>
              ...
              <div className="flex items-center gap-4 bg-slate-900/80 p-4 rounded-xl border border-slate-700/80">
                ...
                <div
                  className={`px-3 py-1.5 rounded-lg text-xs font-semibold border ${getScoreBadge(auditData.overall_risk_score).bg}`}
                >
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              {auditData.audits.map((audit: ModelAuditResult, idx: number) => {
                ...
                return (
                  <div
                    key={idx}
                    className="bg-slate-800/90 border border-slate-700 rounded-2xl p-6 shadow-xl flex flex-col justify-between space-y-6"
                  >
                    {/* Header Info */}
                    <div className="space-y-4">
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <div className="bg-slate-900/60 p-3.5 rounded-xl border border-slate-700/60 space-y-2">
                          <div className="flex justify-between items-center text-xs">
                            ...
                            <span className="font-bold text-slate-200">
                              {audit.microaggression_score} / 10
                            </span>
                          </div>
                          <div className="w-full bg-slate-800 rounded-full h-2 overflow-hidden">
                            <div
                              ...
                              style={{
                                width: `${(audit.microaggression_score / 10) * 100}%`,
                              }}
                            />
                          </div>
                        </div>
                        ...
                        <div className="bg-slate-900/60 p-3.5 rounded-xl border border-slate-700/60 space-y-2">
                          <div className="flex justify-between items-center text-xs">
                            ...
                            <span className="font-bold text-slate-200">
                              {audit.linguistic_erasure_score} / 10
                            </span>
                          </div>
                          <div className="w-full bg-slate-800 rounded-full h-2 overflow-hidden">
                            <div
                              ...
                              style={{
                                width: `${(audit.linguistic_erasure_score / 10) * 100}%`,
                              }}
                            />
                          </div>
                        </div>
                      ...
                      <div className="space-y-1.5">
                        <span className="text-xs text-slate-400 uppercase tracking-wider font-semibold">
                          Detected Bias Markers
                        </span>
                        <div className="flex flex-wrap gap-1.5">
                          {audit.bias_categories_detected.map((cat, cIdx) => (
                            <span
                              key={cIdx}
                              className="text-xs bg-indigo-500/10 text-indigo-300 border border-indigo-500/20 px-2.5 py-1 rounded-md font-medium"
                            >
                              {cat}
                            </span>
                          ))}
                      ...
                      <div className="space-y-1 text-sm bg-slate-900/40 p-3.5 rounded-xl border border-slate-700/40">
                        <span className="text-xs text-slate-400 uppercase tracking-wider font-semibold block">
                          Pragmatic Subtext Audit
                        </span>
                        <p className="text-amber-200/90 italic text-xs sm:text-sm leading-relaxed">
                          &quot;{audit.subtext_audit}&quot;
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="border-t border-slate-700/80 pt-4 space-y-3">
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex gap-1 bg-slate-900 p-1 rounded-lg border border-slate-700/80">
                          {(["corporate", "formal", "informal"] as const).map(
                            (mode) => (
                              <button
                                key={mode}
                                onClick={() =>
                      ...
                      <div className="relative p-4 bg-slate-900/90 border border-slate-700/80 rounded-xl text-xs sm:text-sm text-slate-200 font-mono leading-relaxed">
                        <p className="pr-12">
                          {audit.suggested_responses[currentTab]}
                        </p>
                        <button
                          onClick={() =>
                            handleCopy(
                              audit.suggested_responses[currentTab],
                              `${audit.model_name}-${currentTab}`,
                            )
                          }
                        ...
                        >
                          {copiedKey === `${audit.model_name}-${currentTab}`
                            ? "Copied! ✓"
                            : "Copy"}
                        </button>
                      </div>
